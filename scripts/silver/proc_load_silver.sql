...
-- Should return 0 rows if dedup worked (no duplicate customer IDs)
SELECT cst_id, COUNT(*) FROM silver.crm_cust_info GROUP BY cst_id HAVING COUNT(*) > 1;

-- Should show only 'Single', 'Married', 'n/a' — not raw 'S'/'M' codes
SELECT DISTINCT cst_marital_status FROM silver.crm_cust_info;

-- Should show real product line names, not raw codes
SELECT DISTINCT prd_line FROM silver.crm_prd_info;

-- Row count sanity check — should roughly match your bronze count
